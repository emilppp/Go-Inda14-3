...
func Julia(f ComplexFunc, n int) image.Image {
	...
	wg.Add(n * n) // En waitgroup med antalet pixlar.
	for i := bounds.Min.X; i < bounds.Max.X; i++ {
		for j := bounds.Min.Y; j < bounds.Max.Y; j++ {
			go func(z, x int) { // Kör en go function för alla 'pixelritningar'
				n := Iterate(f, complex(float64(z)/s, float64(x)/s), 256)
				r := uint8(0)
				g := uint8(0)
				b := uint8(n % 32 * 8)
				img.Set(z, x, color.RGBA{r, g, b, 255})
				wg.Done()
			}(i, j) // Skickar med iterationsvariablerna i & j.
		}
	}
	wg.Wait()
	return img
